...
./electrum-sv -v=debug --no-password-check -wp=pass create_jsonrpc_wallet -w demo > "/root/.electrum-sv/$d/create_log.txt" 2>&1 &
sleep 2
echo "loading wallet..."
./electrum-sv daemon -v=debug load_wallet -w demo --walletpassword=pass > "/root/.electrum-sv/$d/load_log.txt" 2>&1 &
...
do
  echo "$(status_code) - syncing headers, checking again in 5 seconds..."
  sleep 5
  status_code=$(curl -s -o /dev/null -w "%{http_code}" --request POST --url http://127.0.0.1:8332/ --header 'Content-Type: text/plain' --data '{"method": "walletpassphrase","params":["pass", 10000],"id": "unlock"}')
done
echo "service signup..."
# now we may continue
./electrum-sv daemon -v=debug service_signup -w demo --walletpassword=pass > "/root/.electrum-sv/$d/signup_log.txt" 2>&1 &
